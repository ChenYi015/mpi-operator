...
func NewMPIJobController(
	...
	mpiJobInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, new interface{}) {
			...
			if !(job.Status.LauncherStatus == kubeflow.LauncherSucceeded || job.Status.LauncherStatus == kubeflow.LauncherFailed) || job.DeletionTimestamp != nil {
...
func newLauncher(mpiJob *kubeflow.MPIJob, kubectlDeliveryImage string) *batchv1.Job {
	...
	container.Resources.Limits = mpiJob.Spec.LauncherResources.Limits
	container.Resources.Requests = mpiJob.Spec.LauncherResources.Requests
